...
class HisSystem(Action):
    ...
    def binganshouye(self, idx, blh):
        ...
        if self.validator.filter_data(basic_info):
            bingan = {
                30102: p('LabSex'),
                30178: p('LabAge'),
                30111: p_1('drNation'),
    ...
    def main(self, blh):
        ...
        for i in idx:
            ...
            shouye = self.binganshouye(i, blh)
            # if not shouhe:
            #     return 
            if shouye:  # 过滤掉不合格胡的数据
                binganshouye.append(shouye)
    ...
    def __call__(self, args, io):
        ...
        name = ybzl['xingming']
        csrq = ybzl['chushengriqi']
        sfzh = ybzl['shengfengzhenghao']
        blh = ybzl['blinglihao']
        jzkh = ybzl['juzhenkahao']
        obj_keys = [30119, 30104, 0, 30101, 30110]
        self.validator = Filter(sfzh, blh, jzhk, name, csrq, obj_keys)

        result = []
        if blh:
            result.append(self.main(blh))
        if name:
            blh_array = list(set(self.zonghechaxun(name)))
            if blh in blh_array: blh_array.remove(blh)
            if blh_array:
                
                result.extend([self.main(i) for i in blh_array])

        content1 = new_content()
        content1.push_group(result)
...
class YihuiSystem(Action):
    ...
    def __call__(self, args, io):
        ...
        blh = ybzl['blinglihao']
        jzkh = ybzl['juzhenkahao']
        obj_keys = ['shengfengzhenghao', 'blinglihao', 'juzhenkahao', 'xingming', 'chushengriqi']

        result = []
        if blh:
            result.append(self.get_data(blh))
        if name:
            blh_array = list(set(self.search_patient_with_name(name)))
            if blh in blh_array: blh_array.remove(blh)
            if blh_array:
                obj = Filter(sfzh, blh, jzhk, name, csrq, obj_keys)
                valid_data = filter(obj.filter_data, [self.patient_info(i) for i in blh_array])
                result.extend([self.get_data(i['binglihao']) for i in valid_data])
