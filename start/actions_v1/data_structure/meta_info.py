class NodeMetaInfo(object):
	def __init__(self, node_id, key, key_path, data_type, data_source, children, is_list, unique_key, order_key, special_rule):
...
class TreeMetaInfo(object):
	...
	def init(self, config):
		...
		for node_id in sorted(config.keys()):
			order_config[node_id] = config[node_id]

		temp_child_2_parent = {}
		for node_id, node_info in order_config.items():
			node_id = int(node_id)
			key = node_info['field_key']
			key_path = [key]
			...
			children = node_info.get('sub_field_ids', [])
			...
			node = NodeMetaInfo(node_id, key, key_path, data_type, source, children, is_list, unique_key, order_key, special_rule)
